...
import { LayoutDashboard, Menu, Info, BarChart3, Table2, Calendar, ChevronDown, Sun, Moon, Monitor } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className={`w-full h-full flex flex-col items-center justify-center ${resolvedTheme === 'dark' ? 'bg-slate-950' : 'bg-white'} text-indigo-500 gap-4`}>
        <div className="animate-spin text-4xl">
           <LayoutDashboard />
        </div>
        <p className={`${resolvedTheme === 'dark' ? 'text-slate-400' : 'text-slate-700'} text-sm`}>Caricamento dati in corso...</p>
      </div>
    );
  }

  return (
    <div className={`flex h-screen w-full ${resolvedTheme === 'dark' ? 'bg-slate-950 text-slate-200' : 'bg-white text-slate-900'} overflow-hidden`}>
      {/* Sidebar */}
      <div className={`flex-shrink-0 transition-all duration-300 ease-in-out h-full border-r ${resolvedTheme === 'dark' ? 'border-slate-800' : 'border-slate-300'} ${isSidebarOpen ? 'w-80' : 'w-0 overflow-hidden'}`}>
        <FilterPanel 
          filters={filters} 
      ...
      <main className="flex-1 flex flex-col h-full relative min-w-0">
        ...
        {/* Header */}
        <header className={`${resolvedTheme === 'dark' ? 'bg-slate-950/90 border-slate-800' : 'bg-white/95 border-slate-300'} backdrop-blur-sm z-10 border-b flex-shrink-0`}>
            ...
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-4">
                {!isSidebarOpen && (
                  <button 
                    ...
                    className={`p-2 ${resolvedTheme === 'dark' ? 'bg-slate-800 hover:bg-slate-700 text-slate-300' : 'bg-slate-200 hover:bg-slate-300 text-slate-800'} rounded-md transition-colors`}
                    title="Apri filtri"
                  >
                ...
                <div>
                  ...
                  <div className={`flex items-center gap-4 text-xs ${resolvedTheme === 'dark' ? 'text-slate-300' : 'text-slate-800'} flex-wrap`}>
                    <span className="flex items-center gap-1">
                      <span className="text-indigo-400 font-bold">{filteredData.length}</span>
                      <span className="font-medium">articoli trovati</span>
                    ...
                    {filters.dateRange.start && filters.dateRange.end && (
                      <span className="flex items-center gap-1">
                        ...
                        <span className={resolvedTheme === 'dark' ? 'text-slate-400' : 'text-slate-700'}>Periodo:</span>
                        <span className={`${resolvedTheme === 'dark' ? 'text-slate-200' : 'text-slate-900'} font-semibold`}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative group">
                  <button
                    className={`p-2 rounded-lg border ${resolvedTheme === 'dark' ? 'border-slate-700 hover:border-slate-600 bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-slate-200' : 'border-slate-400 hover:border-slate-500 bg-white hover:bg-slate-100 text-slate-700 hover:text-slate-900'} transition-all`}
                  ...
                  <div className={`absolute right-0 top-full mt-2 w-40 ${resolvedTheme === 'dark' ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-300'} border rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50`}>
                    <div className="p-1">
                      <button
                        onClick={() => setTheme('light')}
                        className={`w-full px-3 py-2 text-xs text-left rounded-md flex items-center gap-2 transition-colors ${
                          theme === 'light' 
                            ? 'bg-indigo-500/20 text-indigo-300' 
                            : resolvedTheme === 'dark' ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-800 hover:bg-slate-200'
                        }`}
                      >
                ...
                <div className="hidden md:block text-right">
                  <p className={`text-[10px] ${resolvedTheme === 'dark' ? 'text-slate-400' : 'text-slate-700'} font-mono uppercase tracking-wider font-medium`}>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[11px] border transition-all flex items-center gap-1.5 ${
                      view === 'dashboard'
                        ...
                        : resolvedTheme === 'dark' ? 'bg-slate-900 border-slate-700 text-slate-400 hover:border-slate-500 hover:text-slate-200' : 'bg-white border-slate-400 text-slate-800 hover:border-slate-500 hover:text-slate-900'
                    }`}
          ...
          <div className={`px-6 py-3 border-t ${resolvedTheme === 'dark' ? 'border-slate-800 bg-slate-900/50' : 'border-slate-300 bg-white/50'}`}>
            <ActiveFilters 
              filters={filters}
        ...
        <div className="flex-1 overflow-y-auto scroll-smooth p-6">
           <div className="max-w-7xl mx-auto space-y-6">
             ...
             <div className={`${resolvedTheme === 'dark' ? 'bg-amber-900/30 border-amber-500/30' : 'bg-amber-50 border-amber-300'} border rounded-lg overflow-hidden`}>
               <details className="group">
                 <summary className={`px-4 py-3 cursor-pointer flex items-center justify-between ${resolvedTheme === 'dark' ? 'hover:bg-amber-900/20' : 'hover:bg-amber-100'} transition-colors`}>
                   <div className="flex items-center gap-2">
                     <Info className={`w-4 h-4 ${resolvedTheme === 'dark' ? 'text-amber-400' : 'text-amber-600'} flex-shrink-0`} />
                     <span className={`text-sm font-semibold ${resolvedTheme === 'dark' ? 'text-amber-200' : 'text-amber-900'}`}>
                       Informazioni sulle Immagini
                     </span>
                   </div>
                   <ChevronDown className={`w-4 h-4 ${resolvedTheme === 'dark' ? 'text-amber-400' : 'text-amber-600'} transition-transform group-open:rotate-180`} />
                 </summary>
                 <div className="px-4 pb-4 pt-2">
                   <p className={`text-xs ${resolvedTheme === 'dark' ? 'text-amber-200/80' : 'text-amber-900/90'} leading-relaxed`}>
                     Tutte le immagini sono state scaricate e controllate in locale dall'<strong>og:image</strong>. 
                     Il default di tutte le immagini dagli <code className={`${resolvedTheme === 'dark' ? 'bg-amber-900/50' : 'bg-amber-200'} px-1 py-0.5 rounded`}>og:width</code> è <strong>1200px</strong>.
                   </p>
                 </div>
               </details>
             </div>
             ...
             {view === 'dashboard' ? (
               <>
                 ...
                 {/* Section Title */}
                 <div className="mb-4">
                   <h2 className={`text-xl font-bold ${resolvedTheme === 'dark' ? 'text-slate-100' : 'text-slate-900'} flex items-center gap-2`}>
                     <BarChart3 className="w-5 h-5 text-indigo-400" />
                     Dashboard Analitico
                   </h2>
                   <p className={`text-sm ${resolvedTheme === 'dark' ? 'text-slate-300' : 'text-slate-800'} mt-1`}>
                     Visualizzazioni interattive dei dati filtrati. <span className="text-indigo-400 font-semibold">Clicca sui grafici</span> per applicare filtri rapidi.
                   </p>
                 </div>
                 ...
                 {trashItems.length > 0 && (
                   <div className="mb-4 mt-8">
                     ...
                     <p className={`text-sm ${resolvedTheme === 'dark' ? 'text-slate-300' : 'text-slate-800'} mt-1`}>
                       Top 10 immagini con problemi di qualità, dimensioni o compressione. <span className="text-amber-400 font-semibold">Priorità: immagini con bordo rosso</span>.
                     </p>
                   </div>
                 )}
                 ...
                 {/* Trash Table */}
                 {trashItems.length > 0 ? (
                   <TrashTable data={trashItems} theme={resolvedTheme} />
                 ) : (
                   <div className={`${resolvedTheme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-300'} border rounded-xl p-8 text-center`}>
                     <p className={`${resolvedTheme === 'dark' ? 'text-slate-300' : 'text-slate-800'} text-sm font-medium`}>✅ Nessuna immagine critica trovata nel periodo selezionato.</p>
                     <p className={`${resolvedTheme === 'dark' ? 'text-slate-400' : 'text-slate-700'} text-xs mt-2`}>Tutte le immagini rispettano i parametri di qualità.</p>
                   </div>
                 )}
               </>
             ) : (
               <>
                 ...
                 <div className="mb-4">
                   ...
                   <p className={`text-sm ${resolvedTheme === 'dark' ? 'text-slate-300' : 'text-slate-800'} mt-1`}>
                     Elenco completo degli articoli con tutti i dettagli. <span className="text-sky-400 font-semibold">Scorri verso il basso</span> per caricare automaticamente più risultati.
                   </p>
                 </div>
          ...
          {/* Footer */}
          <footer className={`py-8 text-center ${resolvedTheme === 'dark' ? 'text-slate-600' : 'text-slate-600'} text-xs mt-4 space-y-1`}>
            <div>
              DataScope Analytics v1.0 &copy; {new Date().getFullYear()}
            </div>
            <div className={`text-[11px] ${resolvedTheme === 'dark' ? 'text-slate-500' : 'text-slate-600'}`}>
              DataScope – Scraper{' '}
              <a
